...
d = pickle.load(open('spring2.p', 'rb'))
encpos = d['position']
encvel = d['velocity']
encD =d['duty']
encdir =d['direction']
...
plt.title('Damper: Time Domain Response')
ax1.legend(['Angular Position', 'Duty Cycle (%)'], loc='upper left')
ax2.legend(['Velocity'])
plt.grid(True)

plt.figure()
plt.title('Damper: Motor Response')
...
plt.figure()
plt.title('Damper: Motor Velocity Response')
plt.plot(encvel, encdir*encD*100, color=blue)
plt.ylabel('Directional Duty Cycle (%)')

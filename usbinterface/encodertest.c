...
void VendorRequests(void) {
    ...
    switch (USB_setup.bRequest) {
        ...
        case READ_SW1:
            ...
            break;
        case ENC_READ_REG:
            angresult = (WORD32)joy.angle;
            // result = enc_readReg(USB_setup.wValue);
            BD[EP0IN].address[0] = angresult.b[0];
            BD[EP0IN].address[1] = angresult.b[1];
            BD[EP0IN].address[2] = angresult.b[2];
            BD[EP0IN].address[3] = angresult.b[3];
            BD[EP0IN].bytecount = 4;         // set EP0 IN byte count to 1
            BD[EP0IN].status = 0xC8;         // send packet as DATA1, set UOWN bit
            break;
        case TOGGLE_LED3:
            led_toggle(&led3);
        ...
        case READ_SW3:
            ...
            break;
        case WRITE_KS:
            spring_k = USB_setup.wValue;
            BD[EP0IN].address[0] = spring_k.b[0];
            BD[EP0IN].address[1] = spring_k.b[1];
